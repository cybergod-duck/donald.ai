export default async function handler(req, res) {
  ...
  try {
    ...
    if (!text) throw new Error('No speech text returned from LLM');

    // Single-pass cleanup + cheering enforcement
    text = text
      .replace(/\r\n/g, '\n')
      .replace(/\b[A-Za-z0-9]{6,}\b(?=.*\d)/g, '') // More targeted gibberish removal
      .replace(/[ \t]{2,}/g, ' ')
      .replace(/\n{3,}/g, '\n\n')
      .trim();

    const cheeringCount = (text.match(/\[cheering\]/gi) || []).length;
    if (cheeringCount === 0) {
      ...
      let kept = false;
      text = text.replace(/\[cheering\]/gi, () => kept ? '' : (kept = true, '[cheering]'));
    }
