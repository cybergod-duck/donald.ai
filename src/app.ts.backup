document.addEventListener('DOMContentLoaded', () => {
  ...
  function updateMusicUI() {
    ...
    elements.musicIcon.src = `${ASSET_BASE}images/note-${isMusicOn ? 'on' : 'off'}.png`;
  }
  ...
  function switchVideo(videoFile, loop = false) {
    if (!elements.visual || elements.visual.src.endsWith(videoFile)) return;
    const isSpecial = videoFile.startsWith('special/');
    const subdir = isSpecial ? 'special/' : 'mouth-shapes/';
    const fullPath = `${ASSET_BASE}videos/${subdir}${videoFile.replace(/^.*\//, '')}`;
  ...
  function setTranscript(text) {
    elements.transcript.innerHTML = text.split('\n\n').map(p => `<p>${p.replace(/\n/g, '<br>')}</p>`).join('');
  }
  ...
  function playMidCheer(callback) {
    ...
    switchVideo(getRandomVideo('closed'));
    setTimeout(() => {
      isCheering = false;
      callback();
    }, 6000);
  ...
  async function playSpeech(audioChunks, transcript) {
    try {
      const fullBlob = new Blob(audioChunks.map(b64 => Uint8Array.from(atob(b64), c => c.charCodeAt(0))), { type: 'audio/mpeg' });
